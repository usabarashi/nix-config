...
writeShellScriptBin "claude" ''
  ...
  # Nix access (cloud-restricted.sb): hand the guard shim the workspace target
  # and ephemeral Nix state/config/cache dirs inside the per-invocation temp
  # tree. The pinned nix binary/version are rendered into the deployed shim by
  # home-manager (not exported here). Not exported when there is no flake root,
  ...
  # Git over HTTPS authenticates with the dedicated read-only GitHub PAT, never
  # the personal Keychain credential. The nixpkgs git system config sets
  # credential.helper=osxkeychain; command-scope config (GIT_CONFIG_*) resets
  # that list and binds github.com to `gh auth git-credential`, with
  ...
  # ~/.config/git stay unreadable, and GIT_TERMINAL_PROMPT=0 plus the askpass
  # scrub prevent a prompt fallback to the personal identity. GIT_ALLOW_PROTOCOL
  # keeps the exec-allowed HTTP remote helper from being used for plaintext
  # HTTP. When PAT provisioning was skipped or failed (informational args, or
  # AGENT_GH_ALLOW_UNAUTHENTICATED=1), the helper is a failing command so git
  # fetch cannot silently fall back to `gh`'s Keychain lookup and authenticate
