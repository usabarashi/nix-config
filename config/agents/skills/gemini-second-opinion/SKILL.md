...
- `Gemini CLI is not running in a trusted directory` (exit 55) — the snippet is missing `GEMINI_CLI_TRUST_WORKSPACE=true`. Add it to the `gemini` invocation. Prefer the env var over the `--skip-trust` flag and do not combine them: the env var is the more stable contract across Gemini CLI versions, since flag/env semantics have shifted between releases, and combining the two would mask a regression in the env-var path.
